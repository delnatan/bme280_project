...
echo "Required packages: fastapi uvicorn smbus2 bme280"

# Check if packages are available
python3 -c "import fastapi, uvicorn, smbus2, bme280" 2>/dev/null
if [ $? -eq 0 ]; then
    echo "✓ All Python dependencies are available"
else
    echo "⚠ Some dependencies missing. Install with:"
    echo "  pip3 install --user fastapi uvicorn[standard] smbus2 bme280"
    echo "  OR try: sudo apt install python3-fastapi python3-uvicorn"
    echo ""
